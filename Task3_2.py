...
'''
def repeat_Astar(current_energy_met, current_path_found, current_path, current_dist, current_cost, straight_dict, dist_cost_dict, start, end, max_cost):
    temp_result = (current_energy_met, current_path_found, current_path, current_dist, current_cost)
    for i in range(1,len(current_path)):
        try:
            node1 = current_path[i-1]
            node2 = current_path[i]
            ...
            if energy_met and path_found and (temp_result[4] > max_cost or dist < temp_result[3]):
                temp_result = energy_met, path_found, path, dist, cost
                
        except IndexError:
            continue
    
    if temp_result == (current_energy_met, current_path_found, current_path, current_dist, current_cost and not (energy_met and path_found)):
...
def execute(start, end, energy):
    ...
    if not energy_met:
        if path_found:
            ...
            energy_met, path_found, path, dist, cost = repeat_Astar(energy_met, path_found, path, dist, cost, Task3_straight_dict, Task3_dist_cost_dict, start, end, energy)
        else:
            print("No paths found between the node pair.")
    
    # Can instead repeat constrained_Astar even if energy_met: Greatly increases execution time, but a better path may be found
    #energy_met, path_found, path, dist, cost = repeat_Astar(energy_met, path_found, path, dist, cost, Task3_straight_dict, Task3_dist_cost_dict, start, end, energy)
