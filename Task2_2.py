...
'''
def repeat_UCS(current_energy_met, current_path_found, current_path, current_dist, current_cost, dist_cost_dict, start, end, max_cost):
...
def execute(start, end, energy):
    ...
    if not energy_met:
        if path_found:
            ...
            energy_met, path_found, path, dist, cost = repeat_UCS(energy_met, path_found, path, dist, cost, Task2_dict, start, end, energy)
        else:
            print("No paths found between the node pair.")
    
    # Can instead repeat constrained_UCS even if energy_met: Greatly increases execution time, but a better path may be found
